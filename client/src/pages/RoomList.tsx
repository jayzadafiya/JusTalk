      ...
      setRooms((prevRooms) => {
        ...
        if (existingRoomIndex !== -1) {
          ...
          if (currentUserId) {
            ...
            const participantIds = room.participants.map((p: any) => p?._id);
            const isParticipant = participantIds.includes(user._id);
            if (isParticipant) {
    ...
    <div className="flex-1 flex flex-col bg-slate-900 text-white">
      <div className="p-6 pb-[50px]">
        <div className="grid grid-cols-1 md:grid-cols-2 items-center mb-6 gap-3">
          ...
          {rooms.length > 0 && (
            <div className="flex flex-row gap-3 items-stretch md:justify-end">
              <Button
                fullWidth
                className="md:w-auto"
